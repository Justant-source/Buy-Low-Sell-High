...
.PHONY: bootstrap-check lint-docs test smoke test-data test-strategy test-backtest test-manual test-integration typecheck lint ci dashboard-build dashboard-test e2e clean-room migrate worker-smoke scenario-report e2e-backtest e2e-manual e2e-risk backup restore-test reference-check backtest-reference backtest-grid backtest-run official-explorer official-matrix official-reference-check legacy-mentor-compare mentor-matrix parity-mentor-matrix mentor-floor data-import data-sync data-validate dashboard worker docker-init docker-sync docker-backtest
...
backtest-run:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m soxl_mania.cli backtest run --profile configs/strategies/ddeolsao_pal_official_v1.yaml --symbol SOXL
...
reference-check backtest-reference official-reference-check:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -c 'from soxl_mania.config import load_strategy_config; from soxl_mania.data.normalize import normalize_bars; from soxl_mania.data.providers.csv_provider import CsvMarketDataProvider; from soxl_mania.data.quality import compute_data_hash; from soxl_mania.reporting.official_explorer import build_official_explorer; from soxl_mania.reporting.official_matrix import build_official_matrix, compare_to_reference, load_explorer_reference_fixture, load_reference_fixture; bars = normalize_bars(CsvMarketDataProvider("data/raw/soxl_daily_2011_present.csv").load_bars("SOXL")); config = load_strategy_config("configs/strategies/ddeolsao_pal_official_v1.yaml", initial_capital=10000); data_hash = compute_data_hash(bars); matrix = build_official_matrix(bars, config, data_hash=data_hash); explorer = build_official_explorer(bars, config, data_hash=data_hash); matrix_result = compare_to_reference(matrix, load_reference_fixture()); explorer_result = compare_to_reference(explorer, load_explorer_reference_fixture()); print("matrix", matrix_result["status"]); print("matrix_first_mismatch", matrix_result["first_mismatch"]) if matrix_result["first_mismatch"] else None; print("explorer", explorer_result["status"]); print("explorer_first_mismatch", explorer_result["first_mismatch"]) if explorer_result["first_mismatch"] else None; raise SystemExit(0 if matrix_result["status"] == "PASS" and explorer_result["status"] == "PASS" else 1)'

backtest-grid:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m soxl_mania.cli backtest grid --profile configs/strategies/ddeolsao_pal_official_v1.yaml --csv engine/tests/fixtures/sample_soxl.csv --threads 5,6,7 --stops 10,30,40

official-explorer:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m soxl_mania.cli backtest official-explorer --profile configs/strategies/ddeolsao_pal_official_v1.yaml --csv data/raw/soxl_daily_2011_present.csv --symbol SOXL

official-matrix:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m soxl_mania.cli backtest official-matrix --profile configs/strategies/ddeolsao_pal_official_v1.yaml --csv data/raw/soxl_daily_2011_present.csv --symbol SOXL --threads 5,6,7 --stops 10,30,40
...
scenario-report:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) -m soxl_mania.cli backtest risk-report --profile configs/strategies/ddeolsao_pal_official_v1.yaml --csv engine/tests/fixtures/sample_soxl.csv --symbol SOXL
...
ci: clean-room backup-restore-test
