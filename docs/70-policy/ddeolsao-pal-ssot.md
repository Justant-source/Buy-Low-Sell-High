...
이 화면의 thread 상태는 모두 `end-of-session` 기준으로 해석한다. 즉 같은 세션에 진입 후 청산된 thread는 그 세션의 `open_positions`에 남지 않는다.
...
| 항목 | 상태 |
|---|---|
| 전체 parity | `DATA_MISMATCH` 가능 |
| 권위 기준 | 멘토 전사 fixture |
| 현재 용도 | `legacy comparison` 진단 |
| CI 게이트 여부 | 사용 안 함 |
| 공식 기준 | Yahoo `official-explorer` / `official-matrix` |
...
1. **이 문서가 SSOT다.** 매매 로직, 파라미터 해석, 청산 우선순위는 모두 4절과 5절을 따른다.
2. **보유 기간은 거래소 세션(거래일) 기준이다.** 달력일 기준 계산 금지.
3. **결정론적 실행.** 동일 설정(`config_hash`) + 동일 데이터(`data_hash`) → 동일 결과. 난수·시각 의존 로직 금지.
4. **`ideal_same_close`는 공식 연구 baseline이다.** 실전 수익 예측과 수동 체결 검증은 별도 경로로 분리한다.
...
| 문서 | 역할 |
|---|---|
| `docs/90-adr/0003-official-research-baseline.md` | 공식 Yahoo 연구 기준선 (ADR) |
