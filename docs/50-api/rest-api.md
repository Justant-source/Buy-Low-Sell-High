...
- `GET /api/workspaces`
  ...
  - 현재 `soxl`과 `0193t0`를 반환한다.
  - `0193t0`는 `referenceMode=backtest_only`이며, SOXL 전용 멘토/공식 참조 탭을 숨기는 데 사용한다.
...
- `GET /api/backtests/strategy-explorer`
  ...
  - `0193T0` 기본값은 `ideal_same_close` + `raw_close_with_actions`다.
  ...
  - UI 랭킹 정렬은 선택 slice의 `full_return_pct desc`를 우선하고, 동률일 때만 `mean_segment_return_pct desc`, `segment_stddev_pct asc`를 사용한다.
...
- `GET /api/backtests/official-explorer`
  ...
  - 이 경로는 현재 SOXL reference workspace에만 의미가 있다.
