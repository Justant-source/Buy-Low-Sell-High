...
- 새 작업은 하나의 Phase 또는 워크스트림 게이트에 묶어 진행하고, 관련 없는 UI/엔진/데이터 변경을 같은 패치에 섞지 않는다.
- 구현 계획의 초기 Phase 설명이 현재 코드보다 오래되었더라도, 현재 구현된 later-phase 기능을 삭제하거나 축소하지 않는다.
- `make` alias는 유지하되, 문서에는 필요한 직접 명령도 함께 남겨 `make`가 없는 환경을 허용한다.

## AI Agent 작업 절차
- 시작 시 `git status --short`로 기존 수정분을 확인하고, 대상 파일의 현재 내용을 다시 읽는다.
- 변경 전 `docs/_index.md`의 변경 유형별 읽기 경로에서 관련 문서를 확인한다.
- 코드 변경과 정책 변경이 함께 필요하면 같은 작업 안에서 문서와 구현을 맞춘다.
- 레퍼런스 또는 golden 실패는 fixture를 바꾸기 전에 첫 mismatch, `data_hash`, `config_hash`, `code_commit`를 기록한다.
- 최종 보고에는 실행한 Phase Gate 명령과 결과를 포함한다.
