...
- 워크스트림 `7-8`은 Express 대시보드, workspace 기반 UI, CLI 기반 API, 파일 기반 대시보드 작업 산출물을 포함한다.
- 로컬 TypeScript 검증은 `npm --prefix dashboard run build` 와 `npm --prefix dashboard test`로 실행 가능하다.
- 로컬 대시보드 스모크 플로우는 `make e2e-backtest`, `make e2e-risk`로 실행 가능하다.
- Phase 8 리스크 리포트는 `make scenario-report` 와 `make e2e-risk` 경로를 통해 로컬 실행 가능하다.
- Phase 9 종합 게이트는 `make clean-room`, `make ci`로 실행 가능하다.
